...
def deploy(config, target_user, target_home):
    ...
    # Download Claude Code installer (no sudo)
    files.download(
    ...
    # Install Claude Code (user-local, no sudo)
    claude_installed = host.get_fact(Command, f"test -d {target_home}/.claude && echo yes || echo no")
    if claude_installed != "yes":
        server.shell(
            name="Install Claude Code",
            commands=["bash /tmp/claude-install.sh"],
            _env={
                "HOME": target_home,
                "USER": target_user,
            },
    ...
    if current_shell != "/usr/bin/zsh":
        server.shell(
            ...
            commands=[f"chsh -s /usr/bin/zsh {target_user}"],
        )
